...
<body>
					...
					<tr>
						...
						<td class="m-0 p-0">
							<select class="form-control" name="search_employee_id">
								...
								if(!empty($_POST['seartch_department_id'])){
									$sql_department = " AND employee.department_id = '" . $_POST['seartch_department_id'] ."' "; //POSTあり
								}else{
									if(isset($_COOKIE['department_id'])){
										$sql_department = " AND employee.department_id = '" . $_COOKIE['department_id'] ."' "; //POSTなし、Cookieあり
									}else{
										$sql_department = ""; //POSTなし、Cookieなし
									}
								}

								if(!empty($_POST['search_group_id'])){
									$sql_group = " AND employee.group_id = '" . $_POST['search_group_id'] ."' "; //POSTあり
								}else{
									if(isset($_COOKIE['group_id'])){
										$sql_group = " AND employee.group_id = '" . $_COOKIE['group_id'] ."' "; //POSTなし、Cookieあり
									}else{
										$sql_group = ""; //POSTなし、Cookieなし
									}
								}
						...
						<td class="m-0 p-0">
							<select class="form-control" name="search_department_id">
								<option value="" <?php if(!isset($_POST['search_department_id']) && !isset($_COOKIE['department_id'])){echo "selected";} ?>>(指定なし)</option>
								<?php
								try{
								...
								if($count>0){
									while($row=$stmh->fetch(PDO::FETCH_ASSOC)){
										...
										<option value="<?=htmlspecialchars($row['department_id'],ENT_QUOTES)?>"
											<?php
											if(!isset($_POST['search_department_id']) && isset($_COOKIE['department_id'])){
												if($_COOKIE['department_id'] == htmlspecialchars($row['department_id'],ENT_QUOTES)){ //POSTは未定義だがCookieはあるとき
													echo "selected";
												}
											}elseif(isset($_POST['search_department_id'])){
												if($_POST['search_department_id'] == htmlspecialchars($row['department_id'],ENT_QUOTES)){ //POSTが定義されているとき
													echo "selected";
												}
											}
											?>
								...
								if($count>0){
									while($row=$stmh->fetch(PDO::FETCH_ASSOC)){
										...
										<option value="<?=htmlspecialchars($row['group_id'],ENT_QUOTES)?>"
											<?php
											if(!isset($_POST['search_group_id']) && !empty($_COOKIE['employee_id'])){
												if($_COOKIE['group_id'] == htmlspecialchars($row['group_id'],ENT_QUOTES)){ //POST宣言なし、Cookieあり
													echo "selected";
												}
											}elseif(isset($_POST['search_group_id'])){
	...
	if(isset($_POST['search_end_date'])){
		...
		$sql_search_end_date = " AND zangyo_date <= '" . date('Y-m-d', strtotime('+3 day')) ." 23:59:59'";
	}
	...
	if(isset($_POST['search_department_id'])){
		if(!empty($_POST['search_department_id'])){
			$sql_search_department_id = " AND department.department_id = " . $_POST['search_department_id']; //宣言あり、値あり
		}else{
			$sql_search_department_id = ""; //宣言あり、値なし
		}
	}else{
		if(isset($_COOKIE['department_id'])){
			$sql_search_department_id = " AND department.department_id = " . $_COOKIE['department_id']; //POST宣言なし、Cookieあり
		}else{
			$sql_search_department_id = ""; //POST宣言なし、Cookieなし
		}
	}

	if(isset($_POST['search_group_id'])){
		if(!empty($_POST['search_group_id'])){
			$sql_search_group_id = " AND work_group.group_id = " . $_POST['search_group_id']; //宣言あり、値あり
		}else{
			$sql_search_group_id = ""; //宣言あり、値なし
		}
	}else{
		if(isset($_COOKIE['group_id'])){
			$sql_search_group_id = " AND work_group.group_id = " . $_COOKIE['group_id']; //POST宣言なし、Cookieあり
		}else{
			$sql_search_group_id = ""; //POST宣言なし、Cookieなし
		}
	}
