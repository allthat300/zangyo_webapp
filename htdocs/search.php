...
<body>
					...
					<tr>
						...
						<td class="m-0 p-0">
							<select class="form-control" name="search_employee_id">
								<option value="" <?php if(!isset($_POST['search_employee_id']) && !isset($_COOKIE['employee_id'])){echo "selected";} ?>>(指定なし)</option>
								<?php

								if(!empty($_POST['search_department_id'])){
									$sql_department = " AND employee.department_id = '" . $_POST['search_department_id'] ."' ";
								}else{
									$sql_department = "";
								}
								...
								if($count>0){
									while($row=$stmh->fetch(PDO::FETCH_ASSOC)){
										...
										<option value="<?=htmlspecialchars($row['employee_id'],ENT_QUOTES)?>"
											<?php
											if(!isset($_POST['search_employee_id']) && !empty($_COOKIE['employee_id'])){ //宣言なし、Cookieあり
												if($_COOKIE['employee_id'] == htmlspecialchars($row['employee_id'],ENT_QUOTES)){
													echo "selected";
												}
											}elseif(isset($_POST['search_employee_id'])){ //宣言あり
												if($_POST['search_employee_id'] == htmlspecialchars($row['employee_id'])){
													echo "selected";
												}
											}
											?>
	...
	if(isset($_POST['search_end_date'])){
		...
		$sql_search_end_date = " AND zangyo_date <= '" . date('Y-m-d', strtotime('+7 day')) ." 23:59:59'";
	}
	if(isset($_POST['search_employee_id'])){
		if(!empty($_POST['search_employee_id'])){
			$sql_search_employee_id = " AND employee.employee_id = " . $_POST['search_employee_id']; //宣言あり、値あり
		}else{
			$sql_search_employee_id = ""; //宣言あり、値なし
		}
	}else{
		if(isset($_COOKIE['employee_id'])){
			$sql_search_employee_id = " AND employee.employee_id = " . $_COOKIE['employee_id']; //宣言無し、Cookieあり
		}else{
			$sql_search_employee_id = ""; //宣言無し、Cookieなし
		}
	}
	if(!empty($_POST['search_department_id'])){
